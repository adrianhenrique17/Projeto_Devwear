...
// ✅ Mock correto sem sobrescrever SENHA_REGEX
...
describe("updateUser (unitário) - Validações", () => {
  ...
  describe("Validação de força da senha", () => {
    const senhasInvalidas = [
      ...
      "Senha@", // sem número e < 8
      "12345678", // apenas números
      "senha@forte", // sem maiúscula
    ];
  ...
  describe("Validação de CPF", () => {
    const cpfsInvalidos = [
      "00000000000", // repetido
      "123456", // muito curto
      "52998224724", // dígito verificador inválido
      "111.222.333-44", // formatado inválido
    ];
    ...
    test("Deve rejeitar CPF válido mas diferente do original", async () => {
      const req = {
        ...
        body: {
          name: "Nome Válido",
          password: "Senha@123",
          cpf: "86328422708", // válido, mas diferente do mock
        },
      } as Request<{ id: string }>;
  ...
  describe("Validação de CPF com função externa", () => {
    test.each([
      ...
      ["08225031062", false], // ✅ substituto confiável
      ["12345678909", true], // válido matematicamente
      ["00000000000", false],
      ["11111111111", false],
      ["5299822472", false], // curto
      ["529982247256", false], // longo
